...
async def save_url(bot, message, cb=False):
    ...
    if save_filename and len(save_filename)>60:
        return await client.sendMessage(chat_id, Translation.LONG_CUS_FILENAME)
    ...
    if not size :
        return await sent_msg.edit(Translation.FILE_SIZE_ERROR)
    if size>(2*1000*1000*1000) :
        return await sent_msg.edit(Translation.MAX_FILE_SIZE)
